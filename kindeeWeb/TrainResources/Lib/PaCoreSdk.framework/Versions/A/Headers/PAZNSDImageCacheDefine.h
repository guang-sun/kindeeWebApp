...
/// Image Cache Type
typedef NS_ENUM(NSInteger, PAZNImageCacheType) {
    /**
     ...
     */
    PAZNImageCacheTypeAll
};

typedef void(^PAZNImageCacheCheckCompletionBlock)(BOOL isInCache);
typedef void(^PAZNImageCacheCalculateSizeBlock)(NSUInteger fileCount, NSUInteger totalSize);
typedef NSString * _Nullable (^PAZNImageCacheAdditionalCachePathBlock)(NSString * _Nonnull key);
typedef void(^PAZNImageCacheQueryCompletionBlock)(UIImage * _Nullable image, NSData * _Nullable data, PAZNImageCacheType cacheType);
typedef void(^PAZNImageCacheContainsCompletionBlock)(PAZNImageCacheType containsCacheType);

/**
 This is the built-in decoding process for image query from cache.
 @note If you want to implement your custom loader with `queryImageForKey:options:context:completion:` API, but also want to keep compatible with PAZNSDWebImage's behavior, you'd better use this to produce image.
 ...
 */
FOUNDATION_EXPORT UIImage * _Nullable PAZNImageCacheDecodeImageData(NSData * _Nonnull imageData, NSString * _Nonnull cacheKey, PAZNSDWebImageOptions options, PAZNSDWebImageContext * _Nullable context);

/**
 This is the image cache protocol to provide custom image cache for `PAZNSDWebImageManager`.
 Though the best practice to custom image cache, is to write your own class which conform `PAZNSDMemoryCache` or `PAZNSDDiskCache` protocol for `PAZNImageCache` class (See more on `PAZNImageCacheConfig.memoryCacheClass & PAZNImageCacheConfig.diskCacheClass`).
 However, if your own cache implementation contains more advanced feature beyond `PAZNImageCache` itself, you can consider to provide this instead. For example, you can even use a cache manager like `PAZNImageCachesManager` to register multiple caches.
 */
@protocol PAZNImageCache <NSObject>

@required
/**
 Query the cached image from image cache for given key. The operation can be used to cancel the query.
 If image is cached in memory, completion is called synchronously, else aynchronously and depends on the options arg (See `PAZNSDWebImageQueryDiskSync`)
 ...
 */
- (nullable id<PAZNSDWebImageOperation>)queryImageForKey:(nullable NSString *)key
                                             options:(PAZNSDWebImageOptions)options
                                             context:(nullable PAZNSDWebImageContext *)context
                                          completion:(nullable PAZNImageCacheQueryCompletionBlock)completionBlock;
...
- (void)storeImage:(nullable UIImage *)image
         ...
         cacheType:(PAZNImageCacheType)cacheType
...
- (void)containsImageForKey:(nullable NSString *)key
                  cacheType:(PAZNImageCacheType)cacheType
                 completion:(nullable PAZNImageCacheContainsCompletionBlock)completionBlock;
...
/**
 ...
 @param completionBlock A block executed after the operation is finished
 */
- (void)clearWithCacheType:(PAZNImageCacheType)cacheType
                completion:(nullable PAZNSDWebImageNoParamsBlock)completionBlock;

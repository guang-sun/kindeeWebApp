...
em_observerPathOnlyNotify(Model, @(((void)Model.ModelPath, #ModelPath)), Target, @(((void)Target.TargetPath, #TargetPath)), NO, 0, YES)
#define EMObserveTagOnlyNotify(Model, ModelPath, Target, TargetPath, Tag) \
em_observerPathOnlyNotify(Model, @(((void)Model.ModelPath, #ModelPath)), Target, @(((void)Target.TargetPath, #TargetPath)), NO, Tag, YES)
...
/**
 ...
 @param Model 监听数据源
 @param ModelPath 监听Model的path
 */
#define EMObserveUnBindingAll(Model) \
em_observerUnBindingAll(Model)
#define EMObserveUnBindingPath(Model, ModelPath) \
em_observerUnBindingPath(Model, ModelPath)
#define EMObserveUnBindingWithPath(Model, ModelPath)\
em_observerUnBindingPath(Model, @(((void)Model.ModelPath, #ModelPath)))
#define EMObserveUnBindingPathTag(Model, ModelPath, Tag) \
em_observerUnBindingPathWithTag(Model, @(((void)Model.ModelPath, #ModelPath)), Tag)
...
void em_observerPathOnlyNotify(id model, NSString *observerPath, id target, NSString *targetPath, BOOL isTargetObject, NSInteger tag,  BOOL isOnlyNotify);

void em_observerPathAction(id model, NSString *observerPath, BFMVVMAction targetAction, NSInteger tag, BOOL isTargetObject);

void em_observerUnBindingAll(id model);
void em_observerUnBindingPath(id model, NSString *path);
void em_observerUnBindingPathWithTag(id model, NSString *path, NSInteger tag);

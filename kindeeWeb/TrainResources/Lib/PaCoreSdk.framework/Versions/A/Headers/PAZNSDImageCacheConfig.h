...
typedef NS_ENUM(NSUInteger, PAZNImageCacheConfigExpireType) {
    /**
     * When the image is accessed it will update this value
     */
    PAZNImageCacheConfigExpireTypeAccesPAZNSDate,
    /**
     * The image was obtained from the disk cache (Default)
     */
    PAZNImageCacheConfigExpireTypeModificationDate
};
 ...
 */
@interface PAZNImageCacheConfig : NSObject <NSCopying>

/**
 Gets the default cache config used for shared instance or initialization when it does not provide any cache config. Such as `PAZNImageCache.sharedImageCache`.
 ...
 */
@property (nonatomic, class, readonly, nonnull) PAZNImageCacheConfig *defaultCacheConfig;
...
/*
 * The option to control weak memory cache for images. When enable, `PAZNImageCache`'s memory cache will use a weak maptable to store the image at the same time when it stored to memory, and get removed at the same time.
 ...
 */
@property (assign, nonatomic) PAZNImageCacheConfigExpireType diskCacheExpireType;
...
/**
 * The custom memory cache class. Provided class instance must conform to `PAZNSDMemoryCache` protocol to allow usage.
 * Defaults to built-in `PAZNSDMemoryCache` class.
 * @note This value does not support dynamic changes. Which means further modification on this value after cache initlized has no effect.
 */
@property (assign, nonatomic, nonnull) Class memoryCacheClass;

/**
 * The custom disk cache class. Provided class instance must conform to `PAZNSDDiskCache` protocol to allow usage.
 * Defaults to built-in `PAZNSDDiskCache` class.
 * @note This value does not support dynamic changes. Which means further modification on this value after cache initlized has no effect.
 */

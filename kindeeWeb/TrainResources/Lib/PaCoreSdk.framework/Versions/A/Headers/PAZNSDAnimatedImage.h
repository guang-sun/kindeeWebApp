...
#import "PAZNSDImageCoder.h"


/**
 This is the protocol for PAZNSDAnimatedImage class only but not for PAZNSDAnimatedImageCoder. If you want to provide a custom animated image class with full advanced function, you can conform to this instead of the base protocol.
 */
@protocol PAZNSDAnimatedImage <PAZNSDAnimatedImageProvider>
 ...
 */
- (nullable instancetype)initWithData:(nonnull NSData *)data scale:(CGFloat)scale options:(nullable PAZNSDImageCoderOptions *)options;

/**
 Initializes the image with an animated coder. You can use the coder to decode the image frame later.
 @note We use this with animated coder which conforms to `PAZNSDProgressiveImageCoder` for progressive animation decoding.
 
 @param animatedCoder An animated coder which conform `PAZNSDAnimatedImageCoder` protocol
 @param scale The scale factor to assume when interpreting the image data. Applying a scale factor of 1.0 results in an image whose size matches the pixel-based dimensions of the image. Applying a different scale factor changes the size of the image as reported by the `size` property.
 @return An initialized object
 */
- (nullable instancetype)initWithAnimatedCoder:(nonnull id<PAZNSDAnimatedImageCoder>)animatedCoder scale:(CGFloat)scale;
...
/**
 The image class which supports animating on `PAZNSDAnimatedImageView`. You can also use it on normal UIImageView/NSImageView.
 */
@interface PAZNSDAnimatedImage : UIImage <PAZNSDAnimatedImage>
 ...
 */
@property (nonatomic, assign, readonly) PAZNSDImageFormat animatedImageFormat;
...
// By default, animated image frames are returned by decoding just in time without keeping into memory. But you can choose to preload them into memory as well, See the decsription in `PAZNSDAnimatedImage` protocol.

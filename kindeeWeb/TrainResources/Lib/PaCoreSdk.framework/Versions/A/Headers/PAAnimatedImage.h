...
extern const NSTimeInterval kPAAnimatedImageDelayTimeIntervalMinimum;

//
//  An `PAAnimatedImage`'s job is to deliver frames in a highly performant way and works in conjunction with `PAAnimatedImageView`.
...
//
@interface PAAnimatedImage : NSObject
...
// Pass either a `UIImage` or an `PAAnimatedImage` and get back its size
+ (CGSize)sizeForImage:(id)image;

// On success, the initializers return an `PAAnimatedImage` with all fields initialized, on failure they return `nil` and an error will be logged.
...
typedef NS_ENUM(NSUInteger, PAAnimatedLogLevel) {
    PAAnimatedLogLevelNone = 0,
    PAAnimatedLogLevelError,
    PAAnimatedLogLevelWarn,
    PAAnimatedLogLevelInfo,
    PAAnimatedLogLevelDebug,
    PAAnimatedLogLevelVerbose
};

@interface PAAnimatedImage (Logging)

+ (void)PASetLogBlock:(void (^)(NSString *logString, PAAnimatedLogLevel logLevel))logBlock logLevel:(PAAnimatedLogLevel)logLevel;
+ (void)PALogStringFromBlock:(NSString *(^)(void))stringBlock withLevel:(PAAnimatedLogLevel)level;

@end

#define PAFLLog(logLevel, format, ...) [PAAnimatedImage PALogStringFromBlock:^NSString *{ return [NSString stringWithFormat:(format), ## __VA_ARGS__]; } withLevel:(logLevel)]

@interface PAZNFLWeakProxy : NSProxy

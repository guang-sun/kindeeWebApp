...
@protocol PAZNTextLinePositionModifier;
 ...
 */
extern const CGSize PAZNTextContainerMaxSize;

/**
 The PAZNTextContainer class defines a region in which text is laid out.
 PAZNTextLayout class uses one or more PAZNTextContainer objects to generate layouts.
 
 A PAZNTextContainer defines rectangular regions (`size` and `insets`) or 
...
/// Creates a container with the specified path. The path.
+ (instancetype)containerWithPath:(nullable UIBezierPath *)path;

/// The constrained size. (if the size is larger than PAZNTextContainerMaxSize, it will be clipped)
...
@property PAZNTextTruncationType truncationType;
...
@property (nullable, copy) id<PAZNTextLinePositionModifier> linePositionModifier;
@end


/**
 The PAZNTextLinePositionModifier protocol declares the required method to modify
 the line position in text layout progress. See `PAZNTextLinePositionSimpleModifier` for example.
 */
...
/**
 ...
 @param lines     An array of PAZNTextLine.
 @param text      The full text.
 @param container The layout container.
 */
- (void)modifyLines:(NSArray<PAZNTextLine *> *)lines fromText:(NSAttributedString *)text inContainer:(PAZNTextContainer *)container;
@end


/**
 A simple implementation of `PAZNTextLinePositionModifier`. It can fix each line's position
...
/**
 PAZNTextLayout class is a readonly class stores text layout result.
...
*/
+ (nullable PAZNTextLayout *)layoutWithContainerSize:(CGSize)size text:(NSAttributedString *)text;
...
/**
 ...
 @return A new layout, or nil when an error occurs.
 */
+ (nullable PAZNTextLayout *)layoutWithContainer:(PAZNTextContainer *)container text:(NSAttributedString *)text;
...
/**
 ...
 @return A new layout, or nil when an error occurs.
 */
+ (nullable PAZNTextLayout *)layoutWithContainer:(PAZNTextContainer *)container text:(NSAttributedString *)text range:(NSRange)range;

/**
 Generate layouts with the given containers and text.
 
 @param containers An array of PAZNTextContainer object (if nil, returns nil).
 @param text       The text (if nil, returns nil).
 @return An array of PAZNTextLayout object (the count is same as containers),
    or nil when an error occurs.
 */
+ (nullable NSArray<PAZNTextLayout *> *)layoutWithContainers:(NSArray<PAZNTextContainer *> *)containers
...
///< The text container
@property (nonatomic, strong, readonly) PAZNTextContainer *container;
///< The full text
...
///< Array of `PAZNTextLine`, no truncated
@property (nonatomic, strong, readonly) NSArray<PAZNTextLine *> *lines;
///< PAZNTextLine with truncated token, or nil
@property (nullable, nonatomic, strong, readonly) PAZNTextLine *truncatedLine;
///< Array of `PAZNTextAttachment`
@property (nullable, nonatomic, strong, readonly) NSArray<PAZNTextAttachment *> *attachments;
 ...
 */
- (nullable PAZNTextPosition *)closestPositionToPoint:(CGPoint)point;
...
/**
 ...
 @return A text position, or nil if not found.
 */
- (nullable PAZNTextPosition *)positionForPoint:(CGPoint)point
                                  oldPosition:(PAZNTextPosition *)oldPosition
                                otherPosition:(PAZNTextPosition *)otherPosition;
 ...
 */
- (nullable PAZNTextRange *)textRangeAtPoint:(CGPoint)point;
...
/**
 ...
 at point. Or nil if not found.
 */
- (nullable PAZNTextRange *)closestTextRangeAtPoint:(CGPoint)point;
 ...
 */
- (nullable PAZNTextRange *)textRangeByExtendingPosition:(PAZNTextPosition *)position;
...
/**
 ...
 @return The line index, or NSNotFound if not found.
 */
- (NSUInteger)lineIndexForPosition:(PAZNTextPosition *)position;
 ...
 */
- (CGPoint)linePositionForPosition:(PAZNTextPosition *)position;
 ...
 */
- (CGRect)caretRectForPosition:(PAZNTextPosition *)position;
...
/**
 ...
 If not found, it returns CGRectNull.
 */
- (CGRect)firstRectForRange:(PAZNTextRange *)range;
...
/**
 ...
 If not found, it returns CGRectNull.
 */
- (CGRect)rectForRange:(PAZNTextRange *)range;
...
/**
 ...
 @return An array of `PAZNTextSelectionRect` objects that encompass the selection.
 If not found, the array is empty.
 */
- (NSArray<PAZNTextSelectionRect *> *)selectionRectsForRange:(PAZNTextRange *)range;
...
/**
 ...
 If not found, the array is empty.
 */
- (NSArray<PAZNTextSelectionRect *> *)selectionRectsWithoutStartAndEndForRange:(PAZNTextRange *)range;
...
/**
 ...
 @param range An object representing a range in text.
 @return An array of `PAZNTextSelectionRect` objects contains the start and end to
 the selection. If not found, the array is empty.
 */
- (NSArray<PAZNTextSelectionRect *> *)selectionRectsWithOnlyStartAndEndForRange:(PAZNTextRange *)range;
...
- (void)drawInContext:(nullable CGContextRef)context
                ...
                debug:(nullable PAZNTextDebugOption *)debug

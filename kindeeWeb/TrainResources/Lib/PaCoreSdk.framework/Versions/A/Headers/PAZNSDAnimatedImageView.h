...
#import "PAZNSDAnimatedImage.h"

/**
 A drop-in replacement for UIImageView/NSImageView, you can use this for animated image rendering.
 Call `setImage:` with `UIImage(NSImage)` which conform to `PAZNSDAnimatedImage` protocol will start animated image rendering. Call with normal UIImage(NSImage) will back to normal UIImageView(NSImageView) rendering
 ...
 */
@interface PAZNSDAnimatedImageView : UIImageView
...
/**
 ...
 @note The best practice to use incremental load is using `initWithAnimatedCoder:scale:` in `PAZNSDAnimatedImage` with animated coder which conform to `PAZNSDProgressiveImageCoder` as well. Then call incremental update and incremental decode method to produce the image.
...
/**
 ...
 Default is NSRunLoopCommonModes on multi-core iOS device, NPAZNSDefaultRunLoopMode on single-core iOS device
 */

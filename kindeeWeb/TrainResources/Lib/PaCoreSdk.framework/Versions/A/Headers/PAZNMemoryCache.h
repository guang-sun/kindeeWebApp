...
/**
 PAZNMemoryCache is a fast in-memory cache that stores key-value pairs.
 ...
 PAZNMemoryCache objects differ from NSCache in a few ways:
 ...
 * It can be configured to automatically evict objects when receive memory 
 ...
 The time of `Access Methods` in PAZNMemoryCache is typically in constant time (O(1)).
 */
@interface PAZNMemoryCache : NSObject
 ...
 */
@property (nullable, copy) void(^didReceiveMemoryWarningBlock)(PAZNMemoryCache *cache);

/**
 A block to be executed when the app enter background.
 The default value is nil.
 */
@property (nullable, copy) void(^didEnterBackgroundBlock)(PAZNMemoryCache *cache);

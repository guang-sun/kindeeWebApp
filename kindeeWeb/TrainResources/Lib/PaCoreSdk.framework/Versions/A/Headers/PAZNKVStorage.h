//
//  PAZNKVStorage.h
//  PAZNCache <https://github.com/ibireme/PAZNCache>
//
...
/**
 PAZNKVStorageItem is used by `PAZNKVStorage` to store key-value pair and meta data.
 Typically, you should not use this class directly.
 */
@interface PAZNKVStorageItem : NSObject
...
/**
 Storage type, indicated where the `PAZNKVStorageItem.value` stored.
 ...
 * If you want to store large number of small datas (such as contacts cache), 
   use PAZNKVStorageTypeSQLite to get better performance.
 * If you want to store large files (such as image cache),
   use PAZNKVStorageTypeFile to get better performance.
 * You can use PAZNKVStorageTypeMixed and choice your storage type for each item.
 
 See <http://www.sqlite.org/intern-v-extern-blob.html> for more information.
 */
typedef NS_ENUM(NSUInteger, PAZNKVStorageType) {
    
    /// The `value` is stored as a file in file system.
    PAZNKVStorageTypeFile = 0,
    
    /// The `value` is stored in sqlite with blob type.
    PAZNKVStorageTypeSQLite = 1,
    
    /// The `value` is stored in file system or sqlite based on your choice.
    PAZNKVStorageTypeMixed = 2,
};



/**
 PAZNKVStorage is a key-value storage based on sqlite and file system.
 Typically, you should not use this class directly.
 
 @discussion The designated initializer for PAZNKVStorage is `initWithPath:type:`. 
 ...
 */
@interface PAZNKVStorage : NSObject
...
@property (nonatomic, readonly) PAZNKVStorageType type;  ///< The type of this storage.
 ...
 */
- (nullable instancetype)initWithPath:(NSString *)path type:(PAZNKVStorageType)type NS_DESIGNATED_INITIALIZER;
...
/**
 ...
 If the `type` is PAZNKVStorageTypeFile, then the item.filename should not be empty.
 If the `type` is PAZNKVStorageTypeSQLite, then the item.filename will be ignored.
 It the `type` is PAZNKVStorageTypeMixed, then the item.value will be saved to file 
 system if the item.filename is not empty, otherwise it will be saved to sqlite.
 
 @param item  An item.
 @return Whether succeed.
 */
- (BOOL)saveItem:(PAZNKVStorageItem *)item;

/**
 Save an item or update the item with 'key' if it already exists.
 
 @discussion This method will save the key-value pair to sqlite. If the `type` is
 PAZNKVStorageTypeFile, then this method will failed.
...
/**
 ...
 @discussion
 If the `type` is PAZNKVStorageTypeFile, then the `filename` should not be empty.
 If the `type` is PAZNKVStorageTypeSQLite, then the `filename` will be ignored.
 It the `type` is PAZNKVStorageTypeMixed, then the `value` will be saved to file
 ...
 */
- (nullable PAZNKVStorageItem *)getItemForKey:(NSString *)key;
 ...
 */
- (nullable PAZNKVStorageItem *)getItemInfoForKey:(NSString *)key;
...
/**
 ...
 @param keys  An array of specified keys.
 @return An array of `PAZNKVStorageItem`, or nil if not exists / error occurs.
 */
- (nullable NSArray<PAZNKVStorageItem *> *)getItemForKeys:(NSArray<NSString *> *)keys;
...
/**
 ...
 @return An array of `PAZNKVStorageItem`, or nil if not exists / error occurs.
 */
- (nullable NSArray<PAZNKVStorageItem *> *)getItemInfoForKeys:(NSArray<NSString *> *)keys;

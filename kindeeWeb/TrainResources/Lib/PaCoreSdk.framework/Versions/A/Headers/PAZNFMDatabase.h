...
#import "PAZNFMResultSet.h"
#import "PAZNFMDatabasePool.h"


#if ! __has_feature(objc_arc)
    #define PAZNFMDBAutorelease(__v) ([__v autorelease]);
    #define PAZNFMDBReturnAutoreleased PAZNFMDBAutorelease

    #define PAZNFMDBRetain(__v) ([__v retain]);
    #define PAZNFMDBReturnRetained PAZNFMDBRetain

    #define PAZNFMDBRelease(__v) ([__v release]);

    #define PAZNFMDBDispatchQueueRelease(__v) (dispatch_release(__v));
#else
    // -fobjc-arc
    #define PAZNFMDBAutorelease(__v)
    #define PAZNFMDBReturnAutoreleased(__v) (__v)

    #define PAZNFMDBRetain(__v)
    #define PAZNFMDBReturnRetained(__v) (__v)

    #define PAZNFMDBRelease(__v)

// If OS_OBJECT_USE_OBJC=1, then the dispatch objects will be treated like ObjC objects
// and will participate in ARC.
// See the section on "Dispatch Queues and Automatic Reference Counting" in "Grand Central Dispatch (GCD) Reference" for details. 
    #if OS_OBJECT_USE_OBJC
        #define PAZNFMDBDispatchQueueRelease(__v)
    #else
        #define PAZNFMDBDispatchQueueRelease(__v) (dispatch_release(__v));
    #endif
#endif
...
typedef int(^PAZNFMDBExecuteStatementsCallbackBlock)(NSDictionary *resultsDictionary);
...
- (BOOL)executeStatements:(NSString *)sql withResultBlock:(PAZNFMDBExecuteStatementsCallbackBlock)block;
...
- (PAZNFMResultSet *)executeQuery:(NSString*)sql, ...;
...
- (PAZNFMResultSet *)executeQueryWithFormat:(NSString*)format, ... NS_FORMAT_FUNCTION(1,2);
...
- (PAZNFMResultSet *)executeQuery:(NSString *)sql withArgumentsInArray:(NSArray *)arguments;
...
- (PAZNFMResultSet *)executeQuery:(NSString *)sql values:(NSArray *)values error:(NSError * __autoreleasing *)error;
...
- (PAZNFMResultSet *)executeQuery:(NSString *)sql withParameterDictionary:(NSDictionary *)arguments;


// Documentation forthcoming.
- (PAZNFMResultSet *)executeQuery:(NSString*)sql withVAList: (va_list)args;

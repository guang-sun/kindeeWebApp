...
static NSInteger const LYSDisableOtherTableViewTag                =   10202; // 禁止手势共存的scrollview需要设置这个tag
...
#define kPageMenuHeight                                               54 // 标签栏高度
...
@class LYSMSubBaseViewController;
@class LYSegmentViewController;

#define LYSegmentCommonViewController(className) \
({ \
    id value = self; \
    UIViewController *viewController = nil; \
    if ( !className && [value isKindOfClass:[UIViewController class]] && ![value isKindOfClass:[LYSMSubBaseViewController class]]) { \
        viewController = (UIViewController *)value; \
    }else if ( className && [value isKindOfClass:[LYSMSubBaseViewController class]] ) { \
        viewController = (UIViewController *)value; \
    }else if ( [value isKindOfClass:[LYSegmentViewController class]] ) { \
        viewController = (UIViewController *)value; \
    }else { \
        for (UIView *view = value; view; view = [view isKindOfClass:[LYSMSubBaseViewController class]] ? ((UIViewController *)view).view.superview : view.superview) { \
            UIResponder *nextResponder = [view nextResponder]; \
            if ( className && [nextResponder isKindOfClass:[LYSMSubBaseViewController class]]) { \
                viewController = (UIViewController *)nextResponder; \
                break; \
            } \
            if ( !className && [nextResponder isKindOfClass:[LYSegmentViewController class]]) { \
                viewController = (UIViewController *)nextResponder; \
                break; \
            } \
        } \

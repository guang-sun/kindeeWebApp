...
typedef NS_ENUM(NSUInteger, PAZNSDImageScaleMode) {
    PAZNSDImageScaleModeFill = 0,
    PAZNSDImageScaleModeAspectFit = 1,
    PAZNSDImageScaleModeAspectFill = 2
};

#if PAZNSD_UIKIT || PAZNSD_WATCH
typedef UIRectCorner PAZNSDRectCorner;
#else
typedef NS_OPTIONS(NSUInteger, PAZNSDRectCorner) {
    PAZNSDRectCornerTopLeft     = 1 << 0,
    PAZNSDRectCornerTopRight    = 1 << 1,
    PAZNSDRectCornerBottomLeft  = 1 << 2,
    PAZNSDRectCornerBottomRight = 1 << 3,
    PAZNSDRectCornerAllCorners  = ~0UL
};
#endif
 ...
 */
- (nullable UIImage *)PAZNSD_resizedImageWithSize:(CGSize)size scaleMode:(PAZNSDImageScaleMode)scaleMode;
 ...
 */
- (nullable UIImage *)PAZNSD_croppedImageWithRect:(CGRect)rect;
 ...
 */
- (nullable UIImage *)PAZNSD_roundedCornerImageWithRadius:(CGFloat)cornerRadius
                                              corners:(PAZNSDRectCorner)corners
 ...
 */
- (nullable UIImage *)PAZNSD_rotatedImageWithAngle:(CGFloat)angle fitSize:(BOOL)fitSize;
 ...
 */
- (nullable UIImage *)PAZNSD_flippedImageWithHorizontal:(BOOL)horizontal vertical:(BOOL)vertical;
 ...
 */
- (nullable UIImage *)PAZNSD_tintedImageWithColor:(nonnull UIColor *)tintColor;
 ...
 */
- (nullable UIColor *)PAZNSD_colorAtPoint:(CGPoint)point;
...
/**
 ...
 @note The rect's width/height should not be smaller than or equal to 0. The minX/minY should not be smaller than 0. The maxX/maxY should not be greater than width/height. Attention this limit is different from `PAZNSD_colorAtPoint:` (point: (0, 0) like rect: (0, 0, 1, 1))
 @note The overhead of object creation means this method is best suited for infrequent color sampling. For heavy image processing, grab the raw bitmap data and process yourself.

 @param rect The rectangle of pixels
 @return The color array for specify pixels, or nil if any error occur
 */
- (nullable NSArray<UIColor *> *)PAZNSD_colorsWithRect:(CGRect)rect;
 ...
 */
- (nullable UIImage *)PAZNSD_blurredImageWithRadius:(CGFloat)blurRadius;
...
/**
 ...
 enough memory).
 */
- (nullable UIImage *)PAZNSD_filteredImageWithFilter:(nonnull CIFilter *)filter;
#endif

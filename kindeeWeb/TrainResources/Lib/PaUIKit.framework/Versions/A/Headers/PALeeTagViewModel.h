...
typedef NS_ENUM(NSUInteger, PALeeTagAlignment) {
    PALeeTagAlignmentImageLeft,
    PALeeTagAlignmentImageRight,
};

@interface PALeeTagViewModel : NSObject
...
@property (nonatomic, assign) CGFloat itemSpacingH; // 图文间距
//@property (nonatomic, assign) UIEdgeInsets titleEdgeInsets; // 文字边界  也会造成文字溢出不能显示全的问题，稍后排查
@property (nonatomic, assign) PALeeTagAlignment tagAlignment; // 图文对其样式

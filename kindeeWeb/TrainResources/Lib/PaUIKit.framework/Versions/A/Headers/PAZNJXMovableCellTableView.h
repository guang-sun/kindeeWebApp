//
//  PAZNJXMovableCellTableView.h
//  PAZNJXMovableCellTableView
//
...
@class PAZNJXMovableCellTableView;

@protocol PAZNJXMovableCellTableViewDataSource <UITableViewDataSource>
 ...
 *  数据源约束: 多组数组嵌套且为可变数组
 */
- (NSMutableArray <NSMutableArray *> *)dataSourceArrayInTableView:(PAZNJXMovableCellTableView *)tableView;
...
@protocol PAZNJXMovableCellTableViewDelegate <UITableViewDelegate>
@optional
/**
 *  The cell that will start moving the indexPath location
 *  长按拖拽cell将要开始移动
 */
- (void)tableView:(PAZNJXMovableCellTableView *)tableView willMoveCellAtIndexPath:(NSIndexPath *)indexPath;
/**
 *  Move cell `fromIndexPath` to `toIndexPath` completed
 *  移动cell换了位置下标
 */
- (void)tableView:(PAZNJXMovableCellTableView *)tableView didMoveCellFromIndexPath:(NSIndexPath *)fromIndexPath toIndexPath:(NSIndexPath *)toIndexPath;
/**
 *  Move cell ended
 *  移动cell结束
 */
- (void)tableView:(PAZNJXMovableCellTableView *)tableView endMoveCellAtIndexPath:(NSIndexPath *)indexPath;
 ...
 */
- (void)tableView:(PAZNJXMovableCellTableView *)tableView tryMoveUnmovableCellAtIndexPath:(NSIndexPath *)indexPath;

/**
 *  Customize the screenshot style of the movable cell
 *  自定义移动的cell截图的样式 加阴影啥的
 */
- (void)tableView:(PAZNJXMovableCellTableView *)tableView customizeMovalbeCell:(UIImageView *)movableCellsnapshot;

/**
 *  Custom start moving cell animation
 *  自定义cell拖拽移动的动画
 */
- (void)tableView:(PAZNJXMovableCellTableView *)tableView customizeStartMovingAnimation:(UIImageView *)movableCellsnapshot fingerPoint:(CGPoint)fingerPoint;

@end

@interface PAZNJXMovableCellTableView : UITableView

@property (nonatomic, weak) id<PAZNJXMovableCellTableViewDataSource> dataSource;
@property (nonatomic, weak) id<PAZNJXMovableCellTableViewDelegate> delegate;

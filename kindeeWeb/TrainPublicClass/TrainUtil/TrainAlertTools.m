...
#import "UIAlertController+TrainWindow.h"

#pragma mark - UIAlertView类别扩充
//类别，扩充属性
@interface UIAlertView (TrainAlertTools)
@property (nonatomic, copy) CallBackBlock clickBlock;
@end

//runtime内联机制，实现set/get方法
@implementation UIAlertView (TrainAlertTools)
...
//类别，扩充属性
@interface UIActionSheet (TrainAlertTools)
@property (nonatomic, copy) CallBackBlock clickBlock;
@end

//runtime内联机制，实现set/get方法
@implementation UIActionSheet (TrainAlertTools)
- (void)setClickBlock:(CallBackBlock)block
{
    objc_setAssociatedObject(self, @selector(clickBlock), block, OBJC_ASSOCIATION_COPY);
}
...
#pragma mark - 接口
@interface TrainAlertTools () <UIAlertViewDelegate, UIActionSheetDelegate>

{
    UIAlertView             *mainAlertView;
    UIAlertController       *mainAlertController;
    UIActionSheet           *mainSheetView;
}
...
@implementation TrainAlertTools
...
+ (void)showTipAlertViewWith:(UIViewController *)viewController
                 ...
                 buttonStyle:(TrainAlertActionStyle)btnStyle
{
    if (iOS_Version >= 8.0)
    {
        ...
        if (btnTitle) {
            ...
            switch (btnStyle) {
                case TrainAlertActionStyleDefault:
...
//        [viewController presentViewController:alertController animated:YES completion:nil];
        ...
        if (btnTitle == nil) {
            //此时self指本类
            [self performSelector:@selector(dismissAlertController:) withObject:alertController afterDelay:TrainAlertViewShowTime];
        }
    }
    else
    {
        ...
        //如果没有按钮，自动延迟消失
        if (btnTitle == nil) {
            [self performSelector:@selector(dismissAlertView:) withObject:alert afterDelay:TrainAlertViewShowTime];
        }
    }
}
                      ...
                      message:(NSString *)message
{
    ...
    else
    {
        ...
        //如果没有按钮，自动延迟消失
        [self performSelector:@selector(dismissActionSheet:) withObject:actionSheet afterDelay:TrainAlertViewShowTime];
    }
}
